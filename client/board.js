...
(function() {
    ...
    Game.board.createBoard = function(part1, part2) {
        ...
        if (!Game.board.boardsInfos[part1 + '-' + part2]) {
            throw new Error('Game.board.createBoard: invalid parts combination! (must be "A-C", "A-D", "B-C" or "B-D")');
        }
     ...
     */
    Game.board.drawHexaGrid = function(context, side, width, height) {
        var c = side;
        var a = c / 2;
        ...
        for (i = 0; i < width; i++) {
            for (j = 0; j < height; j++) {
                ...
                context.moveTo(b, 0);
                context.lineTo(0, a);
                context.lineTo(0, a + c);
                context.lineTo(b, 2 * c);
                context.lineTo(2 * b, a + c);
                context.lineTo(2 * b, a);
                context.closePath();
                context.fill();
